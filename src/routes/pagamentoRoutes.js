...
// Rota POST para processar pagamento (somente para não-anúncios)
router.post('/processar', verificarToken, async (req, res) => {
  ...
  // Bloqueia se for anúncio (use rota de anúncios)
  if (type === 'anuncio') {
    return res.status(400).json({ sucesso: false, mensagem: 'Use /api/anuncios/:id/pagar para pagamentos de anúncios.' });
  }
...
// Listar pagamentos do usuário logado com validade e status dinâmico (adaptado para anúncios)
router.get("/meus", verificarToken, async (req, res) => {
  try {
    ...
    const pagamentosComValidade = pagamentos.map(pag => {
      ...
      if (pag.tipoPagamento === 'anuncio') {
        // Para anúncios: pacote como 'anuncio_2s' → 2 semanas = 14 dias
        const semanas = parseInt(pag.pacote.split('_')[1]) || 0;
        diasDeValidade = semanas * 7;
      } else {
        // Para assinaturas: anual=365, mensal=30
        const nomePacote = pag.pacote?.toLowerCase().trim();
        diasDeValidade = nomePacote === "anual" ? 365 : 30;
      }
...
// Verificar assinatura ativa (adaptado, ignora anúncios)
router.get("/assinatura/ativa", verificarToken, async (req, res) => {
  try {
    const usuarioId = req.usuario.id;

    const pagamentoMaisRecente = await Pagamento.findOne({ 
      usuarioId,
      tipoPagamento: { $ne: 'anuncio' }  // Ignora anúncios
